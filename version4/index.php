 ...
 */
function regenerateStaticBoardPages(PDO $db, string $siteTitle, int $threadsPerPage, string $staticDir): void {
    ...
    for ($page = 1; $page <= $totalPages; $page++) {
        ...
        $filename    = $staticDir . '/' . ($page === 1 ? 'index.html' : $page . '.html');
 ...
 */
function regenerateStaticThreadPage(PDO $db, int $threadID, string $siteTitle, string $staticDir): void {
    ...
    $filename      = $staticDir . '/thread_' . $threadID . '.html';
    file_put_contents($filename, $html);
}
...
if (isset($_POST['submit_post'])) {
    ...
    try {
        ...
        // Process image upload if provided (for new threads only)
        ...
        // If a reply, update bumped timestamp of the parent
        if ($parent !== 0) {
        ...
        regenerateStaticBoardPages($db, $siteTitle, $threadsPerPage, $staticDir);
        if ($parent !== 0) {
            regenerateStaticThreadPage($db, $parent, $siteTitle, $staticDir);
        } else {
            regenerateStaticThreadPage($db, $postID, $siteTitle, $staticDir);
        }

        // Redirect to dynamic page (or to the static file if desired)
        if ($parent === 0) {
            redirect("?");
        } else {
 ...
 ****************************************************/
// If a specific thread is requested
...
// Main board view (dynamic fallback)
ob_start();
echo '<div id="newPostForm" style="display: none;" class="form-box">';
echo '<form action="" method="POST" enctype="multipart/form-data">';
echo '<input type="hidden" name="csrf_token" value="' . $_SESSION['csrf_token'] . '">';
